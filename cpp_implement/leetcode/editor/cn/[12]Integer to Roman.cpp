...
//
//
//  Related Topics 哈希表 数学 字符串 👍 1379 👎 0
...
class Solution {
   public:
    std::string intToRoman(int num) {
        ...
        roman_nums.push_back({1000, "M"});
        roman_nums.push_back({900, "CM"});
        roman_nums.push_back({500, "D"});
        roman_nums.push_back({400, "CD"});
        roman_nums.push_back({100, "C"});
        roman_nums.push_back({90, "XC"});
        roman_nums.push_back({50, "L"});
        roman_nums.push_back({40, "XL"});
        roman_nums.push_back({10, "X"});
        roman_nums.push_back({9, "IX"});
        roman_nums.push_back({5, "V"});
        roman_nums.push_back({4, "IV"});
        roman_nums.push_back({1, "I"});
        int n = roman_nums.size();
        for (int i = 0; i < n; i++) {
            while (num >= roman_nums[i].first) {
                num -= roman_nums[i].first;
                ans += roman_nums[i].second;
            }
        }
        return ans;
    }
...
int main(int argc, char* argv[]) {
    Solution solution;
    std::vector<int> nume_array = {3749, 58, 1994};
    for (auto& num : nume_array) {
        std::cout << num
                  << " convert to roman number:" << solution.intToRoman(num)
                  << std::endl;
    }
    return 0;
}
